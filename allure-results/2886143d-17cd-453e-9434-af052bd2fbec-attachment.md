...
> 35 |         expect(deleteresponse.status()).toBe(201);
     |                                         ^ Error: expect(received).toBe(expected) // Object.is equality
  36 |          console.log(deleteresponse.statusText());
  37 |          expect(deleteresponse.statusText).toBe("Created");
  38 |         console.log("#################################################");
  39 |         const getresponse=await request.get("https://restful-booker.herokuapp.com/booking/"+bookingID)
  40 |         console.log(getresponse.status());
  41 |         console.log(getresponse.statusText());
  42 | })
```